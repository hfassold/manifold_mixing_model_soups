...
# FAH Import the additional JRS files needed for ManifoldMixMS
import JrsUtilModel
# ~FAH
...
def parse_arguments():
    ...
    # FAH Added argument from which model number index to start (for "--download-models")
    parser.add_argument(
        "--download_models_startidx", type=int, default=0,
    )
    # ~FAH
    parser.add_argument(
...
if __name__ == '__main__':
    ...
    if args.download_models:
        ...
        for i in range(args.download_models_startidx, NUM_MODELS):
    ...
    if args.eval_individual_models:
        ...
        for j, model_path in enumerate(model_paths):
            ...
            # FAH Specifiy dataset list
            datasetList = [ImageNet2p, ImageNet, ImageNetV2, ImageNetSketch, ImageNetR, ImageNetA]
            #datasetList = [ ImageNetV2 ]
            for dataset_cls in datasetList:
            # ~FAH
    ...
    if args.greedy_soup:
        ...
        # FAH
        # Note if individual model result file could not be found, you likely called evaluate (it deletes this file)
        print ("Note if individual model results could not be find, it might be because you called 'evaluate-soups'")
        # ~FAH
        ...
        # FAH for debugging, we set NUM_MODELS just to 2
        # NUM_MODELS = 2
        # print ("WARNING: FOR DEBUGGING, I REDUCED NUM_MODELS TO 2 !!!")
        # ~FAH
        for i in range(1, NUM_MODELS):
            ...
            # ('ImageNet2p' in the code) takes roughly 30-40 seconds (on my ML PC)
            # ~FAH
            ...
            if held_out_val_accuracy > best_val_acc_so_far:
                ...
                print(f'Adding this model to soup. New soup is {greedy_soup_ingredients}')

        # Finally, evaluate the greedy soup.
        print(f'\nFinal soup: {greedy_soup_ingredients}\n')
        model = get_model_from_sd(greedy_soup_params, base_model)
        results = {'model_name' : f'greedy_soup'}
        num_workers = args.workers
        # FAH for debugging, set num_workser to 0
        # num_workers = 0
        # ~FAH
        #for dataset_cls in [ImageNetR, ImageNetA, ImageNetSketch, ImageNet2p, ImageNet, ImageNetV2]:
        # original order:
        for dataset_cls in [ImageNet2p, ImageNet, ImageNetV2, ImageNetSketch, ImageNetR, ImageNetA]:
            print(f'Evaluating on {dataset_cls.__name__}.')
            dataset = dataset_cls(preprocess, args.data_location, args.batch_size, num_workers)
            accuracy = test_model_on_dataset(model, dataset)
            results[dataset_cls.__name__] = accuracy
            print(accuracy)
